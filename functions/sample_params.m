...
exclusion_bound = params.exclusion_bound/params.dt;%dont let events get within x bins of eachother. this should be in time
...
%exponential filter
ef = genEfilt_ar(tau,event_samples);
ef_init = ef;
...
noiseless_trace = b*ones(1,T); %initial calcium is set to baseline 
N = length(sti); %number of events in trace
...
diffY = trace - noiseless_trace; %trace - prediction
...
for i = 1:length(times_init)
    ...
    a_init = amps_init(i);
    tau = taus_init(i,:);
    noiseless_trace_ = noiseless_trace;
    ati_ = ati;
    [sti_, noiseless_trace_, diffY_] = ...
        add_event(sti,noiseless_trace,diffY_,efs{i},a_init,tau,trace,tmpi,N+1);

    taus{i} = tau;
    ati_ = [ati_ a_init];
    ati = ati_;
    sti = sti_;
    noiseless_trace = noiseless_trace_;
    N = length(sti); %number of events
...
for i = 1:total_sweeps
    ...
    for ii = 1:event_time_sweeps
        ...
        for ni = 1:N%for each event
            ...
            %create the proposal si_ and noiseless_trace_
            %update logC_ to adjusted
            [si_, noiseless_trace_, diffY_] = removeSpike_ar(si,noiseless_trace,diffY,efs{ni},ai(ni),taus{ni},trace,tmpi,ni);
            [si_, noiseless_trace_, diffY_] = add_event(si_,noiseless_trace_,diffY_,efs{ni},ai(ni),taus{ni},trace,tmpi_,ni);
    ...
    for ii = 1:amp_sweeps
        ...
        for ni = 1:N
            ...
            %set si_ to set of events with the move and noiseless_trace_ to adjusted calcium and update logC_ to adjusted
            [si_, noiseless_trace_, diffY_] = removeSpike_ar(si,noiseless_trace,diffY,efs{ni},ai(ni),taus{ni},trace,si(ni),ni);
            [si_, noiseless_trace_, diffY_] = add_event(si_,noiseless_trace_,diffY_,efs{ni},tmp_a_,taus{ni},trace,si(ni),ni);
    ...
    for ii = 1:baseline_sweeps
        ...
        %set si_ to set of events with the move and noiseless_trace_ to adjusted calcium and update logC_ to adjusted
        [noiseless_trace_, diffY_] = remove_base_ar(noiseless_trace,diffY,tmp_b,trace);   
        [noiseless_trace_, diffY_] = add_base_ar(noiseless_trace_,diffY_,tmp_b_,trace);
    ...
    %perhaps better is to choose smarter removals.
        for ii = 1:adddrop 
            ...
            if ~any(abs(tmpi-sti)<exclusion_bound)
                ...
                [si_, noiseless_trace_, diffY_] = add_event(sti,noiseless_trace,diffY_,ef_init,a_init,tau,trace,tmpi, N+1); %adds all posterior' events at same time
                sti_ = si_;
                ati_ = [ati_ a_init];
                fprob = 1;%1/T(1);%forward probability
                rprob = 1;%1/(N+1);%reverse (remove at that spot) probability
                %accept or reject
                ratio = exp(sum((1./(2*NoiseVar)).*( predAR(diffY_,phi,p,1 ) - predAR(diffY,phi,p,1 ) )))*(rprob/fprob)*e_num_events/(N+1);%(e_num_events(1)/(T(1)-e_num_events(1))); %posterior times reverse prob/forward prob
            ...
            if N>0%i.e. we if have at least one event           
                ...
                tmpi = randi(N);%pick one of the events at random
                sti_ = sti;
                sti_(tmpi) = [];
                ...
                [si_, noiseless_trace_, diffY_] = removeSpike_ar(sti,noiseless_trace,diffY_,efs{tmpi},ati(tmpi),taus{tmpi},trace,sti(tmpi),tmpi);
                sti_ = si_;
                ati_(tmpi) = [];
                ...
                ratio = exp(sum((1./(2*NoiseVar)).*( predAR(diffY_,phi,p,1 ) - predAR(diffY,phi,p,1 ) )))*(rprob/fprob)*N/e_num_events;%((T(1)-e_num_events(1))/e_num_events(1)); 
                if (ratio>1)||(ratio>rand)%accept
                    ati = ati_;
                    sti = sti_;
                    noiseless_trace = noiseless_trace_;
    ...
    for ii = 1:tau1_sweeps
        for ni = 1:N 
            ...
            diffY_ = diffY;
            noiseless_trace_ = noiseless_trace;
            [~, noiseless_trace_, diffY_] = removeSpike_ar(sti,noiseless_trace_,diffY_,efs{ni},ati(ni),taus{ni},trace,sti(ni),ni);
            [~, noiseless_trace_, diffY_] = add_event(sti,noiseless_trace_,diffY_,ef_,ati(ni),tau_,trace,sti(ni),ni);
...
%         keyboard
    ...
    if ~isfield(params,'posterior_data_struct') || strcmp(params.posterior_data_struct,'cells')
        ...
        samples_s{i} = sti; %shared events
%         samples_pr{i} = noiseless_trace; %save calcium traces
    ...
    elseif strcmp(params.posterior_data_struct,'arrays')
        ...
        samples_s = [samples_s sti]; %shared events
        for event_i = 1:N
    ...
    objective = [objective -T/2*log(NoiseVar) + predAR(diffY,phi,p,1 )/(2*NoiseVar) + N*log(e_num_events) - log(factorial(N))];
