function [samples_s, trials, mcmc, params]  = sampleParams_linenoise(trace,tau, Tguess, params)
%parameters
...
NoiseVar_init=2; %inial noise estimate
...
tau2_std = 5/20000/dt; %proposal variance of tau parameters
tau1_min = 0/20000/dt;
tau1_max = 15/20000/dt;
tau2_min = 0/20000/dt;
tau2_max = 100/20000/dt;
...
b_min = 0;
b_max = 30;
exclusion_bound = 0;%dont let bursts get within x bins of eachother. this should be in time
% maxNbursts = 3;%if we want to add bursts, whats the maximum bnumber that we will look for?

line_noise_freq = 60; % in Hz
theta = line_noise_freq*dt; % in 1/samples
...
b_init=median(trace); %initial baseline value
...
baseline = b_init;
phi = phi_init; % offset of line noise
a_s = a_s_init; %amplitude of line noise
...
pr = b_init*ones(1,nBins) + a_s_init*sin(((1:nBins)*theta + phi_init)*2*pi); %initial calcium is set to baseline plus noise
...
for i = 1:nsweeps
    ...
    for ii = 1:1
        ...
        tmp_phi = phi;
        tmp_phi_ = mod(tmp_phi+(phi_std*randn),1); %with bouncing off min and max

        %set si_ to set of bursts with the move and pr_ to adjusted calcium and update logC_ to adjusted
        [pr_, logC_] = remove_line_noise(pr,tmp_phi,a_s,theta,trace);  
        [pr_, logC_] = add_line_noise(pr_,tmp_phi_,a_s,theta,trace); 
        ...
        if ratio>1 %accept
            ...
            pr = pr_;
            logC = logC_;
            phi_std = phi_std + 2*.1*rand*phi_std/sqrt(i);
            phiMoves = phiMoves + [1 1];
        elseif rand<ratio %accept
            ...
            phi_std = phi_std - .1*rand*phi_std/sqrt(i);
            phiMoves = phiMoves + [0 1];
            %reject - do nothing
        end
    end
...
params.b_int=b_init; %initial baseline value
params.a_s_min = a_s_min;
params.a_s_max = a_s_max;
params.a_s_init = a_s_init;
params.phi_init = phi_init;
...
display(['phi: ' num2str(phiMoves(1)/phiMoves(2))])
display(['a_s: ' num2str(a_sMoves(1)/a_sMoves(2))])
