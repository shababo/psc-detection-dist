function time_amp_posteriors = plot_amp_times_posterior(results_file, trace_offset, bin_edges, varargin)
...
%     end
    ...
    for j = 1:length(results(i).trials.times)
        ...
        for k = 1:length(results(i).trials.times{j})
            this_time_ind = ceil(results(i).trials.times{j}(k)) - params.start_ind;
            
%             if this_time_ind < 1 || this_time_ind > duration - 2
%                 continue
%             end
%             this_time_ind
...
%     hold on
    plot_trace_stack(true_signal(params.start_ind:(params.start_ind + params.duration - 1)),trace_offset,bsxfun(@plus,zeros(length(traces),3),[0 0 0]),'-',[.02 5],0,2)
    hold on
