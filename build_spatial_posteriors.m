...
for i = 1:size(posteriors_grid,1)
    for j = 1:size(posteriors_grid,2)
        ...
        if ~isempty(posteriors_grid{i,j})
            ...
            histogram(spatial_posteriors(i,j).amps,[20:20:300])
            axis off
            ylim([0 10000])
%             xlim([20 250])
            ...
            figure(9992)
            subplot1((i-1)*size(posteriors_grid,2) + j);
            histogram(spatial_posteriors(i,j).num_events,0:8)
            axis off
...
%             xlim([0 7])
            ...
            figure(9993)
            subplot1((i-1)*size(posteriors_grid,2) + j);
            histogram(spatial_posteriors(i,j).tau1,[5:5:50])
            axis tight
            axis off
            ylim([0 10000])
%             xlim([5 50])
            ...
            figure(99944)
            subplot1((i-1)*size(posteriors_grid,2) + j);
            histogram(spatial_posteriors(i,j).tau2,[30:20:300])
            axis tight
            axis off
            ylim([0 10000])
%             xlim([30 200])
